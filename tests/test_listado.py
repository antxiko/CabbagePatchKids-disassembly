...
Ninguna necesita el cartucho: se hacen sobre src/cabbagepatch.asm y
src/cabbagepatch.notes. Vigilan que el listado no se degrade sin que nadie se
...
NOTES = os.path.join(RAIZ, "src", "cabbagepatch.notes")
TRACE = os.path.join(RAIZ, "work", "cabbagepatch.trace.json")
...
# Athletic Land NO esta en la lista, y a proposito: este cartucho es su motor
# recompilado y arrastra tres bloques de datos suyos que aqui no llama nadie.
# Nombrarlo, ahi, es el hallazgo. Lo vigila tools/repasa_el_porte.py, que
# comprueba que solo aparezca donde el asunto es ese.
OTROS_JUEGOS = ("Pitfall", "Temptations", "Stardust", "Ale Hop", "Colt 36",
                "Middle Earth", "Monkey", "F-1 Spirit", "Hyper Sports",
                "Hyper Olympic", "Hyper Rally", "Demonia",
...
class TestListado(unittest.TestCase):
    ...
    # Las L_XXXX que quedan son destinos de SALTO dentro de una rutina, no
    # rutinas: ninguna se llama con CALL, y eso lo vigila el test de arriba.
    # Aqui se vigila que no AUMENTEN, que es lo que pasaria si un retrazado
    # destapara codigo nuevo sin anotar.
    TECHO_DE_ETIQUETAS_SUELTAS = 245

    def test_las_etiquetas_sin_nombre_no_aumentan(self):
        sueltas = sorted(set(re.findall(r"L_[0-9A-F]{4}", asm())))
        self.assertLessEqual(len(sueltas), self.TECHO_DE_ETIQUETAS_SUELTAS,
                             "han aparecido %d etiquetas sin bautizar: %s"
                             % (len(sueltas), " ".join(sueltas[:12])))
...
class TestWeb(unittest.TestCase):
    ...
    def test_las_paginas_no_inventan_direcciones(self):
        ...
        with open(os.path.join(RAIZ, "cabbagepatch.rom"), "rb") as f:
            rom = f.read()
        t = Tracer(rom, ORG)
