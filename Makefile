# Cabbage Patch Kids (Konami, MSX1) - desensamblado
#
...
# cubre.
#
# La ROM no se distribuye. Hace falta en la raiz como cabbagepatch.rom, y
# `make comprueba` verifica el sha256.

ROM      = cabbagepatch.rom
SHA      = 114945c770531db3ae61eb330d7ed870a3c00d3a90db2a29a4c134135c2c0707
SRC      = src
WORK     = work
ORG      = 0x4000
TITULO   = CABBAGE PATCH KIDS - Konami - MSX1 - cartucho RC-716 de 16 KB en la pagina 1
...
$(ROM):
	...
	@echo ""
	@echo " Es Cabbage Patch Kids (Konami, RC-716) para MSX, 16384 bytes exactos."
...
$(WORK)/cabbagepatch.trace.json: $(ROM) $(SRC)/cabbagepatch.entries $(SRC)/cabbagepatch.nocode
	@mkdir -p $(WORK)
	python3 tools/z80trace.py $(ROM) $(ORG) $(SRC)/cabbagepatch.entries \
	        $(WORK)/cabbagepatch $(SRC)/cabbagepatch.nocode

trace: $(WORK)/cabbagepatch.trace.json

listado: $(WORK)/cabbagepatch.trace.json $(SRC)/cabbagepatch.notes
	python3 tools/mkasm.py $(ROM) $(ORG) $(WORK)/cabbagepatch.trace.json \
	        $(SRC)/cabbagepatch.notes work/msx.sym $(SRC)/cabbagepatch.asm "$(TITULO)"

# La prueba que decide si el desensamblado es fiable.
verify: $(SRC)/cabbagepatch.asm $(ROM)
	@sh tools/verify_build.sh $(SRC)/cabbagepatch.asm $(ROM) $(ORG)
...
sanity: $(WORK)/cabbagepatch.trace.json
	@echo "=================================================================="
	@echo " ningun byte declarado como datos puede salir como codigo"
	@echo "=================================================================="
	@python3 tools/check_trace.py $(WORK)/cabbagepatch.trace.json $(SRC)/cabbagepatch.nocode
	@python3 tools/check_datos_como_codigo.py $(WORK) $(SRC)
	@echo "=================================================================="
	@echo " ningun punto de entrada puede caer dentro de una zona de datos"
	@echo "=================================================================="
	@python3 tools/check_entradas.py $(SRC)/cabbagepatch.entries $(SRC)/cabbagepatch.notes \
	        $(SRC)/cabbagepatch.nocode
	@echo "=================================================================="
...
densidad:
	@python3 tools/densidad.py $(SRC)/cabbagepatch.asm
...
# Las imagenes de la web NO son capturas: tools/graficos.py rehace la memoria
# de video con las mismas copias que hace el cartucho y dibuja el resultado.
imagenes: $(ROM)
	@mkdir -p docs/imagenes
	python3 tools/graficos.py $(ROM) docs/imagenes
...
clean:
	rm -rf $(WORK)/cabbagepatch.trace.json $(WORK)/cabbagepatch.blocks
